 ...
 *
 * Description: ArrayBuffer based memory-efficient ZIP implementation, this code is based
 * on https://github.com/mrananyan/ZipperJS/blob/master/zipper.js
...
class InZIP
{
    ...
    open()
    {
        ...
        return new Promise((resolve, reject) =>
        {
            ...
            request.onerror = (/** @type {Event} */ e) => reject(Error("File.open, " + e.target.error));
            request.onsuccess = () =>
            {
        ...
        if(this.db)
        {
            return new Promise((resolve, reject) =>
            {
                ...
                transaction.onerror = (e) =>
                    reject(Error("File.write, " + e.target.error));
                for(const ab of abs)
                {
    ...
    read(name)
    {
        if(this.db)
        {
            return new Promise((resolve, reject) =>
            {
                ...
                const chunks = [];
                transaction.objectStore(name).openCursor().onsuccess = (/** @type {IDBVersionChangeEvent} */e) =>
                {
                    const cursor = /** @type {IDBCursorWithValue} */(/** @type unknown */(e.target.result));
                    if(cursor)
                    {
                ...
                transaction.onerror = (e) =>
                    reject(Error("File.objects, " + e.target.error));
    ...
    crc(uint8a, crc)
    {
        ...
        {
            y = (crc ^ uint8a[i]) & 0xff;
            x = /** @type number */ (/** @type unknown */ ("0x" + this.config.CRCTables.substr(y * 9, 8)))
            crc = (crc >>> 8) ^ x;
        }
        return crc ^ -1;
    }
    ...
    async blob()
    {
        ...
        const dirEnd =
            ...
            this.decToHex(this.fileCount, 2) +
            this.decToHex(
                dirData.reduce((/** @type {any} */ p, /** @type {{ byteLength: any; }} */ c) => p + c.byteLength, 0),
                4
            ) +
            this.decToHex(
                fileData.reduce((/** @type {any} */ p, /** @type {{ byteLength: any; }} */ c) => p + c.byteLength, 0),
                4
            ) +
            "\x00\x00";
