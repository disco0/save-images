...
function build()
{
    const custom = elements.save.directory.value.replace(
        /[\\\\/:*?"<>|]/g,
        "_"
    );
    let filename =
        elements.save.filename.value
            .replace(/\.zip/g, "")
            .replace(/[`~!@#$%^&*()|+=?;:'",.<>{}[\]\\/]/gi, "-") + ".zip";
    ...
    return {
        ...
        get zip()
        {
            ...
            if(
                zip === false &&
                elements.counter.save.value > Number(elements.prefs.zip.value)
            )
            {
                zip = window.confirm(
                    "Downloading more than 15 images separately is not recommended. Should I download them as a ZIP archive?"
                );
            }
            return zip;
        },
    ...
    const rtn = objs // size
        ...
        .filter((img) =>
        {
            if(elements.group.dimension.checked)
            {
                ...
                {
                    if(
                        Number(width.min.value) &&
                        Number(width.min.value) > img.width
                    )
                    {
                        return false;
                    }
                    if(
                        Number(width.max.value) &&
                        Number(width.max.value) < img.width
                    )
                    {
                        return false;
                    }
                ...
                {
                    if(
                        Number(height.min.value) &&
                        Number(height.min.value) > img.height
                    )
                    {
                        return false;
                    }
                    if(
                        Number(height.max.value) &&
                        Number(height.max.value) < img.height
                    )
                    {
                        return false;
                    }
        ...
        .filter((img) =>
        {
            ...
            {
                const list = elements.blacklist.input.value
                    .split(/\s*,\s*/)
                    .map((k) => k.toLowerCase())
                    .filter((a) => a);
                return !list.some(
                    (keyword) => img.src.toLowerCase().indexOf(keyword) !== -1
                );
            }
            else
            {
...
function update()
{
    ...
    const index = (elements.counter.save.value = elements.counter.save.textContent = filtered().length);
    document.querySelector("[data-cmd=save]").disabled = index === 0;
    document.querySelector("[data-cmd=save-dir]").disabled =
        index === 0 || !("showDirectoryPicker" in window);
...
chrome.runtime.onMessage.addListener((request, sender, response) =>
{
    if(request.cmd === "progress")
    {
        ...
        elements.counter.progress.value =
            elements.counter.progress.max - request.value;
    }
            ...
            request.images
                ...
                .forEach((img) =>
                {
                    ...
                    const {filename, name} = guess(
                        img,
                        elements.files.mask.value,
                        elements.type.noType.checked
                    );
...
const filename = () =>
{
    ...
    elements.save.filename.value = (
        elements.save.format.value || elements.save.format.placeholder
    )
        .replace("[title]", title)
        ...
        .replace(
            "[navigation-date]",
            navigation ? navigation.toLocaleDateString() : "NA"
        )
        .replace(
            "[navigation-time]",
            navigation ? navigation.toLocaleTimeString() : "NA"
        );
};
    ...
    chrome.runtime.sendMessage(
        {
            ...
            {
                const custom = (
                    elements.files.mask.value || "[name][extension]"
                ).split("[custom=");
                if(custom.length === 2)
                {
            ...
            accuracy: elements.group.accurate.checked
                ? "accurate"
                : "partial-accurate", // no-accurate, partial-accurate, accurate
...
document.addEventListener("click", ({target}) =>
{
    ...
    if(cmd === "save" || cmd === "save-dir")
    {
        ...
        if(len > Number(elements.prefs.max.value))
        {
            if(
                window.confirm(
                    `Are you sure you want to download "${ len }" images?`
                )
            )
            {
                save();
    ...
    else if(cmd === "insert")
    {
        ...
        input.selectionStart = input.selectionEnd =
            end + target.dataset.value.length;
        // trigger persist.js
        input.dispatchEvent(
            new Event("change", {
                bubbles: true,
...
const accurate = () =>
{
    ...
    {
        elements.group.accurate.dataset.checked =
            elements.group.accurate.checked;
    ...
    else
    {
        ...
        elements.group.accurate.checked =
            elements.group.accurate.dataset.checked === "true";
    }
    elements.group.accurate.dispatchEvent(
        new Event("change", {
            bubbles: true,
        })
    );
        ...
        (resp) =>
        {
            if(resp !== true)
            {
                ...
                chrome.runtime.sendMessage({
                    method: "notify",
                    message:
                        "Image links are downloaded to the default download directory",
                });
            }
        }
    );
