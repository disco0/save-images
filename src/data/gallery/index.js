...
// localization
[...document.querySelectorAll(/** @type {"title"} */("[data-i18n]"))].forEach(
    (e) =>
    {
...

{
    ...
    {
        document.body.dataset.top = /** @type {boolean & string} */ (true);
...
document.addEventListener("click", (e) =>
{
    ...
    if(cmd === "rename")
    {
        // Keeping this incase it just explodes with new selector
        ...
        let match = /\[#=*(\d*)\]/.exec(pattern);

        if(match && match.length && isNaN(match[1]) === false)
            offset = Number(match[1]);

        const entries = [...document.querySelectorAll("div.entry :checked")];
    ...
    {
        [...document.querySelectorAll(".entry")].forEach(
            (e) => (e.querySelector("input[type=checkbox]").checked = false)
        );
        document.dispatchEvent(new Event("change"));
    }
    else if(cmd === "download" || cmd === "copy")
    {
        const images = [...document.querySelectorAll(".entry :checked")].map(
            (i) =>
            {
...
{
    copy.disabled = rename.disabled = download.disabled =
        document.querySelector(".entry :checked") === null;
});
